...
INPUT_FILE = ROOT / "montana_memorial_highways.csv"
OUTPUT_FILE = ROOT / "montana_commemorative_highways_filtered.csv"
FLAGGED_FILE = ROOT / "montana_commemorative_highways_flagged.csv"
...
def main():
    ...
    if "Name" not in df.columns:
        raise ValueError("Expected column 'Name' in montana_memorial_highways.csv")

    df["commemorative_keyword"] = df["Name"].apply(has_commemorative_keyword)
    df["person_named"] = df["Name"].apply(is_person_named)
    ...
    with log_file.open("w", encoding="utf-8") as handle:
        ...
        handle.write("State: Montana (validation filter)\n\n")
        ...
        if not flagged.empty:
            ...
            for name in flagged["Name"].head(25).tolist():
                handle.write(f"- {name}\n")
