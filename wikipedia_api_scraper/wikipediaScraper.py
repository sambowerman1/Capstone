...
highways_path = "C:/Users/lucas/Data_Science_Capstone/montana/montana_memorial_highways.csv"
highways = pd.read_csv(highways_path)
names = [clean_name(i) for i in highways["Name"]]
highways["Name"] = names

highways.to_csv("montana/data_with_names2.csv", index=False)

data = [get_person_info(name) for name in names]
df = pd.DataFrame(data)
df_unique = df.drop_duplicates(subset="Name", keep="first")
df_unique.to_csv("montana/scraper_output.csv", index=False)

merged = pd.merge(highways, df_unique, on="Name", how="left")
merged.to_csv("montana/final_output.csv", index=False)
